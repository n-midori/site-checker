...
export default function ProjectPage() {
  ...
  const fetchProject = useCallback(async () => {
    const { data } = await supabase.from("projects").select("*").eq("code", projectCode).single();
    if (data) { setProject(data); setProjectId(data.id); return data.id; }
    return null;
  }, [projectCode]);
  ...
  const fetchMembers = useCallback(async (pid: number) => {
    const { data } = await supabase.from("project_members").select("*").eq("project_id", pid).order("id");
    if (data) {
      setMembers(data);
  ...
  const fetchIssues = useCallback(async (pid: number) => {
    const { data: issuesData } = await supabase.from("issues").select("*").eq("project_id", pid).order("id");
    if (!issuesData) return;
    const ids = issuesData.map((i: any) => i.id);
    const { data: commentsData } = ids.length > 0
      ? await supabase.from("comments").select("*").in("issue_id", ids).order("id")
      : { data: [] as any[] };
  ...
  const fetchStatuses = useCallback(async (pid: number) => {
    const { data } = await supabase.from("project_statuses").select("*").eq("project_id", pid).order("order");
    if (data) setStatuses(data);
  }, []);

  useEffect(() => {
    fetchAllProjects();
    fetchProject().then(async (pid) => {
      if (!pid) { setLoading(false); return; }
      await Promise.all([fetchMembers(pid), fetchIssues(pid), fetchStatuses(pid)]);
      setLoading(false);
  ...
  }, [fetchProject, fetchAllProjects, fetchMembers, fetchIssues, fetchStatuses]);
  ...
  const saveMember = async () => {
    ...
    if (editMember) {
      await supabase.from("project_members").update({ name: memberForm.name, role: memberForm.role }).eq("id", editMember.id);
      setMembers(prev => prev.map(m => m.id === editMember.id ? { ...m, ...memberForm } : m));
    } else {
      const { data } = await supabase.from("project_members").insert({ name: memberForm.name, role: memberForm.role, project_id: projectId }).select().single();
  ...
  const saveStatus = async () => {
    ...
    if (editStatus) {
      await supabase.from("project_statuses").update({ name: statusForm.name, color: statusForm.color }).eq("id", editStatus.id);
      ...
      const { data } = await supabase.from("project_statuses").insert({ name: statusForm.name, color: statusForm.color, order: newOrder, project_id: projectId }).select().single();
  ...
  const moveStatus = async (id: number, direction: -1 | 1) => {
    ...
    const promises = updated.map((s, i) =>
      supabase.from("project_statuses").update({ order: i }).eq("id", s.id)
    );
    ...
    return (
      <div style={{ ...S.app, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 20, marginBottom: 12, color: "#555" }}>プロジェクトが見つかりません</div>
          <button onClick={() => router.push("/projects")} style={{ color: "#2563EB", background: "none", border: "none", fontSize: 14, cursor: "pointer" }}>プロジェクト一覧に戻る</button>
        </div>
      </div>
    );
  }
    ...
    <div style={S.app}>
      <header style={S.header}>
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          <div style={{ ...S.logo, cursor: "pointer" }} onClick={() => router.push("/projects")}><div style={S.logoMark}>✓</div>SiteCheck</div>
          <select style={{ background: "#1E293B", color: "#F8FAFC", border: "1px solid #334155", borderRadius: 6, padding: "4px 10px", fontSize: 12, fontWeight: 600, cursor: "pointer", outline: "none" }}
            value={projectCode} onChange={e => router.push(`/projects/${e.target.value}`)}>
